...
SELECT job_id, REPLACE(job_id, 'ACCOUNT', 'ACCNT') 직업결과
FROM employees;

--예제
SELECT last_name as 이름, LOWER(last_name)as LOWEr적용 , UPPER(last_name) as upper적용, 
UPPER(email) 이메일 , INITCAP(email) inicap적용
FROM employees;
--,
...
--숫자형함수 
--반올림 함수 round (숫자, 자릿수) 자릿수가 없으면 점수로 반올림 
